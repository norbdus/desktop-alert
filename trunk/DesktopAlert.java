...
public class DesktopAlert extends JDialog
{
     ...
     * @param title		Title of the alert
     * @param desc		Description that will appear inside the alert
     * @param image		Description that will appear inside the alert
    ...
    private void initContainer()
	{
		...
		if(image!=null)
		{
			mainPane = new AlertPanel(image, desc, color1, color2, size);
		}
		// when image is null use the constructor that does not need an Image
		else
		{
			mainPane = new AlertPanel(desc, color1, color2, size);
		}
